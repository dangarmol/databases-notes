...
CREATE TRIGGER tratar_referidos
	AFTER INSERT ON usuario
	FOR EACH ROW
		DECLARE
		n_recomendaciones := 0; number;
		recompensa number := 0;
		BEGIN
			IF :NEW.referido IS NOT NULL THEN
				UPDATE usuario SET credito_bono = credito_bono + 10 WHERE nickname = :NEW.nickname;
				SELECT numero_referidos INTO n_recomendaciones FROM usuario WHERE nickname = :NEW.referido;
				IF MOD(n_recomendaciones, 3) = 2  THEN
					recompensa := 100;
				ELSE
					recompensa := 10;
				END IF;
				UPDATE usuario SET credito_bono = credito_bono + recompensa, numero_referidos = numero_referidos + 1 WHERE nickname = :NEW.referido;
			END IF;
	END;